...
export default function TerraceScreen({ navigation }) {
  ...
  const renderTerrace = () => {
    ...
    useEffect (() => {
      let records = [];
      const TerraceRef = ref(db, 'The Terrace');
      onValue(TerraceRef, (snapshot) => {
                ...
                <Pressable
                  ...
                  onPress={() => {
                    if (newRating === "") {
                      ...
                      update(ref(db, 'The Terrace/' + childKey), {number_of_ratings: noOfRatings + 1, rating: aveRatings});
